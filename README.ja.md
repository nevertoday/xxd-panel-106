...
# 🦁 XXD Panel 106｜柔彩ピクセル記

### 写真を魂だけになるまで削り、柔らかな色のピクセルでもう一度記憶する
...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。Panel 106 が各原写真から一回で直接、個別に生成し、別番号の作品や中間結果は使いません。どちらの比較方向も厳密に50:50の二領域だけで、作例のコピーは英語で自動生成します。

<!-- xxd-human-intro:start -->
## 向いている場面と解決できること

写真には多くの細部があっても、記憶に残るのは一つの輪郭、動き、距離、色の塊だけかもしれません。**Panel 106** は、同一性を決める2〜4個の視覚的アンカーを選び、柔らかく明るい幾何学秩序のピクセル記憶へ再構築します。遠くからは上質な抽象構成に見え、近くでは確かにあなたの写真だと分かります。

### こんな場合に向いています

- **複雑な写真でも同一性を失いたくないとき：** 副次的な物と細かな構造を削り、その写真を決める輪郭、姿勢、方向、色関係だけを残します。
- **抽象ポスターにしたいが、無名の装飾にはしたくないとき：** 規則的なグリッド、硬い色面、部分的なディザリング、正負形によって、抽象の中から主体を徐々に現します。
- **安価なキャンディ感のない、清新で癒やされる色がほしいとき：** クリーム白、ピーチピンク、ミルクブルー、ミントグリーンなどを元写真から柔らかく明るい限定色へ再調整します。
- **一つのスタイルを柔軟に納品したいとき：** 上下、左右、デザインのみ、複数比率、四端末壁紙、画像フォルダーの一括処理に対応します。

### 解決できる問題

- 写真を単純に縮小、フィルター処理、物ごとにピクセル化せず、現在の原写真から一回で直接再構築します。
- 2〜4個のアンカーは一つの視覚的中心を共有し、カード、グリッド、コラージュの壁にはなりません。
- 比較モードは厳密に50:50の二領域だけです。上下は現実像が上・デザインが下、左右は現実像が左・デザインが右で、第三の帯をつくりません。

### クイックスタート

> この画像を XXD Panel 106 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 2〜4個の視覚的アンカー · 幾何学ピクセル · モジュール色面 · 硬い輪郭 · 部分ディザリング · 一つの視覚的中心 · 大きな余白 · 柔らかく明るい限定色
...
| 知りたいこと | このスタイルが与えるもの |
|---|---|
| **得られるもの** | 最も重要な2〜4個のアンカーだけを残した、上質で抽象的な「ピクセル記憶」ポスター |
| **一目で分かる特徴** | 規則的なグリッド、モジュール化された柔彩色面、硬い輪郭、部分ディザリング、一つの視覚的中心、大きな余白 |
| **入力をどう尊重するか** | 現実像は同一性、構造、自然な質感を保ち、変換像は大胆に要約しながら決定的な輪郭、姿勢、空間方向、感情を正確に残します |
...
```text
同一性と感情を読む → 決定的な視覚アンカーを2〜4個選ぶ → 副次的な物と細部を削る → 規則的なグリッド、モジュール色面、硬い輪郭へ変換 → 部分ディザリング、正負形、余白で層をつくる → 抽象に見えてもこの写真だと分かる柔彩ピクセル記憶を完成
```

## 完成作で識別しやすい特徴

- 価値の高い視覚アンカーを2〜4個だけ残し、複数の小画面に分けず一つの視覚的中心へまとめます。
- 大きなピクセル形状、規則的なグリッド、硬い輪郭、反復配列、部分ディザリング、正負形で幾何学的階層をつくります。
- 遠くでは上質な抽象、近くでは主体として読めます。葉、窓、顔全体、衣服のしわ、機械構造を一つずつ細密に写しません。
- 元写真の最も柔らかく、明るく、親しみやすい色関係から、クリーム白、ピーチピンク、ミルクブルー、ミントグリーンを軸に透明感のある限定色をつくります。
- 少量のピクセル字やドット字は編集的注記だけに使い、ゲーム画面感、写真の直訳的ピクセル化、カードグリッド、第三の帯、二重のスタイル処理を避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-106-prompt.en.md)
...
上下モードは原文の 3:4 と厳密な 50:50 を保持し、左右モードは同じ等分原則を左右の厳密な 50:50 に移します。二領域コンテナを置き換えるのはデザインのみと壁紙モードだけで、柔彩ピクセル記憶の変換美学は変えません。各成果物では選択された一つのモード契約だけを GPT Image 2 に送ります。

## 組み合わせ可能な4つの出力

`top-bottom`、`left-right`、`design-only`、`wallpaper-pack` は単独でも複数でも選べます。複数選択時も、各モードを独立したプロンプトで別々に生成します。

- `top-bottom`：厳密に同じ高さの50:50二領域だけで、現実画像が上、デザインが下。文字は下領域内に置き、見出し帯、フッター、第三領域を禁止します。
- `left-right`：厳密に同じ幅の50:50二領域だけで、現実画像が左、デザインが右。文字は右領域内に置き、上下構成への変換や第三の帯を禁止します。
- `design-only`：元画像は不可視の参照に限定し、見える要素はすべて当該 Panel のデザイン変換言語に従います。
- `wallpaper-pack`：各端末用にデザイン変換のみの壁紙を独立再構成します。

50:50の二領域分割は Panel 106 固有の制約で、改めて指定する必要はありません。各画面は現在の原写真から一回で直接生成し、中間画像や失敗作を第二段階の入力にしません。必要なら中央位置を監査し、決定論的合成は分割契約を生成で満たせない場合または元写真のピクセル保持が必要な場合だけ使います。
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 106 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
画像モデルが完成画面全体の美的再構成を担当し、二連レイアウトも現在の原写真から完成キャンバス一枚を直接生成して、二回目のスタイル処理を行いません。`scripts/compose_panel.py` は50:50中央監査、条件付き復旧、無劣化ピクセル調整にだけ使い、美的成功は評価しません。
...
/xxd-panel-106 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-106 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 106 を適用し、共通設定は一度だけ解決
/xxd-panel-106 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-106`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-106-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-106-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜106 を連続して掲載し、現在のプロジェクトを太字で示します。
