...
# 🦁 XXD Panel 106｜소프트 컬러 픽셀 기록

### 사진을 영혼만 남도록 덜어 내고, 부드러운 색의 픽셀로 다시 기억합니다
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. Panel 106은 각 원본 사진에서 한 번에 직접, 독립적으로 생성하며 다른 Panel이나 중간 결과를 사용하지 않습니다. 두 비교 방향 모두 정확히 50:50인 두 영역만 사용하고 예시 문구는 영어로 자동 생성합니다.

<!-- xxd-human-intro:start -->
## 어울리는 상황과 해결하는 문제

사진에는 디테일이 많아도 기억에 남는 것은 윤곽 하나, 움직임 한 조각, 거리감, 색면일 수 있습니다. **Panel 106**은 정체성을 결정하는 시각적 앵커 2–4개를 골라 부드럽고 밝으며 기하학적 질서가 또렷한 픽셀 기억으로 재구성합니다. 멀리서는 세련된 추상 구성으로 보이고 가까이서는 여전히 분명히 당신의 사진으로 읽힙니다.

### 이런 경우에 잘 맞습니다

- **복잡한 사진에서도 정체성을 잃고 싶지 않을 때:** 부차적 사물과 자잘한 구조를 덜어 내고 사진을 결정하는 윤곽, 자세, 방향, 색 관계만 남깁니다.
- **추상 포스터이지만 알아볼 수 없는 장식은 싫을 때:** 규칙 그리드, 단단한 색면, 부분 디더링, 양형과 음형으로 추상 속에서 피사체가 서서히 나타나게 합니다.
- **값싼 사탕색 없이 산뜻하고 치유적인 색을 원할 때:** 원본에서 크림 화이트, 피치 핑크, 밀크 블루, 민트 그린 등을 부드럽고 밝은 제한 팔레트로 다시 조율합니다.
- **하나의 스타일로 유연하게 납품해야 할 때:** 상하, 좌우, 디자인 전용, 여러 비율, 네 기기 배경화면, 이미지 폴더 일괄 처리를 지원합니다.

### 해결해 주는 것

- 사진을 단순히 축소하거나 필터를 씌우거나 사물별로 픽셀화하지 않고, 현재 원본에서 한 번에 직접 재구성합니다.
- 2–4개 앵커가 하나의 시각적 중심을 공유하며 카드, 그리드, 콜라주 벽으로 흩어지지 않습니다.
- 비교 모드는 정확히 50:50의 두 영역뿐입니다. 상하는 현실 이미지가 위·디자인이 아래, 좌우는 현실 이미지가 왼쪽·디자인이 오른쪽이며 제3의 띠를 만들지 않습니다.

### 빠른 시작

> 이 이미지를 XXD Panel 106로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 2–4개 시각적 앵커 · 기하학적 픽셀 · 모듈식 색면 · 단단한 윤곽 · 부분 디더링 · 하나의 시각적 중심 · 넓은 여백 · 부드럽고 밝은 제한색
...
| 궁금한 점 | 이 스타일이 제공하는 것 |
|---|---|
| **결과물** | 가장 중요한 2–4개 앵커만 남긴 세련된 추상 ‘픽셀 기억’ 포스터 |
| **대표 특징** | 규칙 그리드, 모듈식 파스텔 색면, 단단한 윤곽, 부분 디더링, 하나의 시각적 중심, 넓은 여백 |
| **원본을 존중하는 방식** | 현실 이미지는 정체성, 구조, 자연스러운 질감을 지키고, 변환 이미지는 과감히 단순화하면서 핵심 윤곽, 자세, 공간 방향, 감정을 정확히 남깁니다 |
...
```text
정체성과 감정 읽기 → 결정적인 시각 앵커 2–4개 선택 → 부차적 사물과 자잘한 디테일 삭제 → 규칙 그리드·모듈식 색면·단단한 윤곽으로 변환 → 부분 디더링·양형과 음형·여백으로 층위 구성 → 추상적으로 보여도 이 사진임을 알아볼 수 있는 소프트 컬러 픽셀 기억 완성
```

## 완성작에서 알아보기 쉬운 특징

- 가치 높은 시각 앵커 2–4개만 남겨 여러 작은 장면으로 나누지 않고 하나의 시각적 중심에 조직합니다.
- 큰 픽셀 형태, 규칙 그리드, 단단한 윤곽, 반복 배열, 부분 디더링, 양형과 음형으로 기하학적 층위를 만듭니다.
- 멀리서는 세련된 추상으로, 가까이서는 피사체로 읽힙니다. 잎, 창문, 얼굴 전체, 옷 주름, 기계 구조를 하나씩 세밀하게 복제하지 않습니다.
- 원본의 가장 부드럽고 밝으며 친근한 색 관계를 크림 화이트, 피치 핑크, 밀크 블루, 민트 그린 중심의 맑은 제한 팔레트로 정제합니다.
- 소량의 픽셀 또는 도트 문자는 편집적 주석으로만 사용하며 게임 화면 느낌, 문자 그대로의 사진 픽셀화, 카드 그리드, 제3의 띠, 두 번째 스타일 처리를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-106-prompt.en.md)
...
상하 모드는 원문 3:4와 정확한 50:50 분할을 유지하고, 좌우 모드는 같은 등분 원칙을 정확한 좌우 50:50으로 옮깁니다. 두 영역 컨테이너를 대체하는 것은 디자인 전용과 배경화면 모드뿐이며, 부드러운 픽셀 기억의 변환 미학은 바꾸지 않습니다. 각 결과물에는 선택된 하나의 모드 계약만 GPT Image 2에 전달합니다.

## 조합 가능한 네 가지 출력

`top-bottom`, `left-right`, `design-only`, `wallpaper-pack`을 하나 이상 선택할 수 있습니다. 여러 모드를 고르면 각 모드를 독립 프롬프트로 따로 생성합니다.

- `top-bottom`: 높이가 정확히 같은 50:50 두 영역만 사용하며 현실 이미지는 위, 디자인은 아래에 둡니다. 문구는 아래 영역 안에 두고 제목 띠, 푸터, 제3 영역을 금지합니다.
- `left-right`: 너비가 정확히 같은 50:50 두 영역만 사용하며 현실 이미지는 왼쪽, 디자인은 오른쪽에 둡니다. 문구는 오른쪽 영역 안에 두고 상하 구성 전환이나 제3의 띠를 금지합니다.
- `design-only`: 원본은 보이지 않는 참조이며, 보이는 모든 요소가 해당 Panel의 디자인 변환 언어를 따릅니다.
- `wallpaper-pack`: 각 기기마다 디자인 변환만으로 된 전체 화면 배경을 독립적으로 재구성합니다.

50:50 두 영역 분할은 Panel 106 자체 규칙이므로 사용자가 다시 요청할 필요가 없습니다. 모든 캔버스는 현재 원본에서 한 번에 직접 생성하며 중간 이미지나 실패 결과를 두 번째 입력으로 쓰지 않습니다. 필요하면 중앙선을 검수하고, 결정론적 합성은 생성이 분할 계약을 지키지 못했거나 원본 픽셀 보존이 필요할 때만 사용합니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 106의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
이미지 모델이 완성 화면 전체의 미학적 재구성을 담당하며 이중 패널도 현재 원본에서 완성 캔버스 한 장을 직접 생성하고 두 번째 스타일 처리를 하지 않습니다. `scripts/compose_panel.py`는 50:50 중앙선 검수, 조건부 복구, 무손실 픽셀 조정에만 사용하며 미학적 성공은 판단하지 않습니다.
...
/xxd-panel-106 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-106 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-106 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-106 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 106 적용, 공통 설정은 한 번만 확정
/xxd-panel-106 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-106`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-106-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-106-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–106을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
