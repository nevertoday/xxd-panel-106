...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 106 从各自原图一次直达、独立生成，不借用其他编号作品或中间结果。两种对照都严格 50:50，只有两个区域；样张使用英文智能配字。
...
```text
读取主体身份与情绪 → 选出 2–4 个决定性的视觉锚点 → 删除次要物体与琐碎细节 → 变成规则网格、模块色块与硬边轮廓 → 用局部 dithering、正负形和留白建立层次 → 完成“看似抽象、仍是这张照片”的柔彩像素记忆
```

## 成品中最容易识别的风格特点

- 只保留 2–4 个高价值视觉锚点，并把它们组织进一个视觉核心，而不是拆成多个小画面。
- 大块像素形状、规则网格、硬边轮廓、重复阵列、局部 dithering 与正负形共同建立几何层次。
- 远看先读成高级抽象构成，近看才逐渐辨认主体；不逐片画叶、不逐扇画窗，也不描摹完整五官、衣褶或机械结构。
- 色彩从原图最柔和、最明亮、最亲和的关系中提炼，形成奶油白、蜜桃粉、牛奶蓝、薄荷绿等方向的清透有限色盘。
- 少量像素字或点阵字只作编辑性介入；拒绝游戏截图感、照片像素化复制、卡片宫格、第三条带和二次风格处理。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-106-prompt.en.md)
